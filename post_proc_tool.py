...
# def ReadPCACFSingleTraj(*, iTraj, NReps, NLength, NEvery = 1, dt, NDim, FileStem):
#   """
#   ReadPCACFSingleTraj read all the PCACFs recorded in trajectory labelled by 
#   iTraj (in range from 0 to NTraj-1), each reps (NReps in total, for each
#   repetition, the cumulative averaged PCACFs are recorded in the files) 
#   and each dimensional elements (including all the diagonal terms, such as 
#   xx, yy, zz, in 3D, and off diagonal terms, such as  xy, xz, yz in 3D),
#   and calculate the cumulative integral, which is the binary diffusion 
#   coefficients. The number of dimensions of the simulation is specified 
#   by NDim, and the FileStem is the base name of the data. The stored 
#   PCACF data are named as "FileStem_D12_{iTraj}.txt".
#   ----------------------------------------------------------------------
#   Input (all are keyword arguments, can be input in any order):
  
#   iTraj: integer, the label for trajectory, range from 0 to NTraj-1.
  
#   NReps: integer, the total number of repetitions. This NReps does not 
#   need to be the same as the VACF NReps, unless tLength = tFreq in the 
...
#   # Initialize,
#   PCACFTemp = np.zeros(((NLength+1)*NReps, int(NDim*(NDim+1)/2)))
#   PCACF     = np.zeros((NReps, NLength+1, int(NDim*(NDim+1)/2)))
#   Diff12    = np.zeros((NReps, NLength+1, int(NDim*(NDim+1)/2)))

#   # Read csv by pandas module and then convert to numpy array, PCACFTemp is
#   # a temporary numpy array that stores all the data from the file. 
#   PCACFTemp[:, :] = pd.read_csv(
#   # file name
#   FileStem + "_" + str(iTraj) + "_D12" + ".txt",
#   # skip header rows and the rows that label which divides the reps
#   skiprows = list(range(NLength+5))+[3+(iRep+1)*(NLength+2) for iRep in range(NReps)],
#   nrows = NReps*(NLength+1), usecols = [3,4,5,6,7,8], delimiter = ' ', header=None
#   ).to_numpy()
    
#   for iRep in range(NReps):
#     # Split data for each reps from PCACFTemp 
#     PCACF[iRep, :, :] = PCACFTemp[iRep*(NLength+1):(iRep+1)*(NLength+1),:]
#     for iDim in range(int(NDim*(NDim+1)/2)):
#       # integrate
#       Diff12[iRep, :, iDim] = CumulateInt(PCACF[ iRep, :, iDim], dt*float(NEvery)) 
#   return PCACF, Diff12

# def ReadPCACFAll(*, NTraj, NReps, NLength, NEvery = 1, dt, NDim, FileStem):
#   """
#   ReadPCACFAll read all the PCACFs recorded in each trajectory (NTraj in 
#   total), each reps (NReps in total, for each repetition, the cumulative
#   averaged PCACFs are recorded in the files) and each dimensional elements
#   (including all the diagonal terms, like xx, yy, zz, and off diagonal
#   terms, like xy, xz, yz), and calculate the cumulative integral, which is
#   the binary diffusion coefficients. The number of dimensions of the 
#   simulation is specified by NDim, and the FileStem is the base name of 
#   the data. The stored PCACF data are named as
#   "FileStem_D12_{iTraj}.txt".
#   ----------------------------------------------------------------------
#   Input (all are keyword arguments, can be input in any order):
  
#   NTraj: integer, the total number of trajectories.
...
#   PCACF: a 4 dimensional numpy array with size 
#   (NTraj, NReps, NLength+1, NDim*(NDim+1)/2), it stores all the PCACF for
#   all the trajectory, repetition and dimensionality for specified number 
#   of timestep. Its axis 0 is the number of trajectory, 1 is the number of
#   repetition, 2 is the number of timestep, 3 is the dimensions. For 2D, 
#   the dimensions are (from 0): xx, yx, yy (if the simulation is in xy plane),
#   for 3D, the dimensions are (from 0): xx, yx, yy, zx, zy, zz.
  
#   Diff12: a 4 dimensional numpy array with size 
#   (NTraj, NReps, NLength+1, NDim*(NDim+1)/2), it stores all the mutual-
#   diffusion coefficients for all the trajectory, repetition and dimensionality 
#   for specified number of timestep. Its axis 0 is the number of tra
#   jectory, 
#   1 is the number of repetition, 2 is the number of timestep, 3 is the 
#   dimensions. For 2D, the dimensions are (from 0): xx, yx, yy (if the 
#   simulation is in xy plane), for 3D, the dimensions are (from 0): 
#   xx, yx, yy, zx, zy, zz.
#   """
  
#   # Initialize,
#   PCACFTemp = np.zeros((NTraj, (NLength+1)*NReps, int(NDim*(NDim+1)/2)))
#   PCACF     = np.zeros((NTraj, NReps, NLength+1, int(NDim*(NDim+1)/2)))
#   Diff12    = np.zeros((NTraj, NReps, NLength+1, int(NDim*(NDim+1)/2)))

  
#   for iTraj in range(NTraj):
#     # Read csv by pandas module and then convert to numpy array, PCACFTemp is
#     # a temporary numpy array that stores all the data from the file. 
#     PCACFTemp[iTraj, :, :] = pd.read_csv(
#     # file name
#     FileStem + "_" + str(iTraj) + "_D12"  + ".txt",
#     # skip header rows and the rows that label which divides the reps
#     skiprows = list(range(NLength+5))+[3+(iRep+1)*(NLength+2) for iRep in range(NReps)],
...
#     for iRep in range(NReps):
#       # Split data for each reps from PCACFTemp 
#       PCACF[iTraj, iRep, :, :] = PCACFTemp[iTraj, iRep*(NLength+1):(iRep+1)*(NLength+1),:]
#       for iDim in range(int(NDim*(NDim+1)/2)):
#         # integrate
#         Diff12[iTraj, iRep, :, iDim] = CumulateInt(PCACF[iTraj, iRep, :, iDim], dt*float(NEvery)) 
#   return PCACF, Diff12
